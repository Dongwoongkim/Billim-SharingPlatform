...
public class CommentController {
    ...
    public ResponseEntity readAllCommentByPostId(
    ...
    public ResponseEntity createCommentWithPostId(
            @ApiParam(value = "Post ID", required = true) @PathVariable Long postId,
            @Valid @RequestBody CommentCreateRequest request) {
        return new ResponseEntity(commentService.createCommentWithPostId(postId, request, getCurrentUsernameCheck()),
                CREATED);
    }

    @ApiOperation(value = "댓글 삭제", notes = "댓글을 삭제합니다.")
    @DeleteMapping("/comments/{commentId}")
    public ResponseEntity deleteCommentByCommentId(
            @ApiParam(value = "삭제할 Comment ID", required = true) @PathVariable Long commentId) {
